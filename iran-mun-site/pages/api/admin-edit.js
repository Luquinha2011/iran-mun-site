...
export default function AdminPanel({ user, logout }) {
  ...
  const prefillEdit = (u) => {
    setEditForm({
      username: u.originalUsername || u.username,
      newUsername: '',
      newName: '',
      newPassword: '',
    })
  ...
  const editUser = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) { showStatus(`✅ Updated successfully`, 'success'); setEditForm({ username: '', newUsername: '', newName: '', newPassword: '' }); fetchUsers() }
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #060d1a 0%, #0a1628 50%, #0d1f35 100%)', fontFamily: "'Source Sans 3', sans-serif", color: 'white' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(0,58,107,0.8), rgba(0,95,142,0.6))', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '20px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => router.push('/')} className="action-btn" style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.7)', padding: '8px 18px', borderRadius: 8, fontSize: 13, cursor: 'pointer', fontFamily: "'Source Sans 3', sans-serif", transition: 'all 0.2s' }}>
              🏠 Home
            </button>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '18px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <button onClick={fetchUsers} style={{ padding: '7px 14px', borderRadius: 7, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'rgba(255,255,255,0.4)', fontSize: 12, cursor: 'pointer', fontFamily: "'Source Sans 3', sans-serif" }}>↻</button>
                </div>
              </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {filteredUsers.map((u, idx) => (
                      <tr key={u.originalUsername || u.username} className="user-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', opacity: u.blocked ? 0.5 : 1, transition: 'background 0.15s' }}>
                        ...
                        <td style={{ padding: '14px 20px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div>
                              <div style={{ fontFamily: 'monospace', fontSize: 13, color: 'rgba(255,255,255,0.85)' }}>{u.username}</div>
                              {u.originalUsername && u.originalUsername !== u.username && (
                                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', marginTop: 2 }}>was: {u.originalUsername}</div>
                              )}
                            </div>
                          </div>
                        </td>
                        <td style={{ padding: '14px 20px', fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>
                          {u.name || <span style={{ color: 'rgba(255,255,255,0.2)', fontStyle: 'italic' }}>—</span>}
                        </td>
                        <td style={{ padding: '14px 20px' }}>
                        ...
                        <td style={{ padding: '14px 20px' }}>
                          {(u.originalUsername || u.username) === PROTECTED_ADMIN ? (
                            ...
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                              {/* EDIT BUTTON — prefills original username */}
                              <button className="action-btn" onClick={() => prefillEdit(u)} style={{ background: 'rgba(0,158,219,0.12)', border: '1px solid rgba(0,158,219,0.3)', color: '#009EDB', padding: '5px 12px', borderRadius: 6, fontSize: 11, cursor: 'pointer', fontWeight: 700, fontFamily: "'Source Sans 3', sans-serif", transition: 'all 0.2s' }}>
                                ✏️
                              </button>
                              {/* BLOCK BUTTON */}
          ...
          {activeTab === 'create' && (
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 32, maxWidth: 600 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                {[
                  { label: 'Username', key: 'username', type: 'text', placeholder: 'e.g. john123', full: false },
                  { label: 'Full Name', key: 'name', type: 'text', placeholder: 'e.g. John Smith', full: false },
                  { label: 'Password', key: 'password', type: 'password', placeholder: 'Strong password', full: true },
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 32, maxWidth: 600 }}>
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)' }}>
                  {editForm.username
                    ? <>Editing: <span style={{ color: '#009EDB', fontWeight: 700 }}>{editForm.username}</span> — leave fields blank to keep current value.</>
                    : 'Click the ✏️ button on a user row to pre-fill this form, or type a username manually.'}
                </div>
              </div>

              {/* CURRENT USERNAME — shows original key, read-only hint */}
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontSize: 11, color: 'rgba(255,255,255,0.4)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>
                  Current Username <span style={{ color: 'rgba(0,158,219,0.6)', fontStyle: 'italic', textTransform: 'none', letterSpacing: 0 }}>(original — used to find the account)</span>
                </label>
                <input
                  className="admin-input"
                  style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', borderColor: 'rgba(0,158,219,0.3)', background: 'rgba(0,158,219,0.06)' }}
                  type="text"
                  placeholder="Type username or click ✏️ on a row"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                {[
                  ...
                  { label: 'New Password', key: 'newPassword', type: 'password', placeholder: 'Leave blank to keep', full: true },
                ].map(({ label, key, type, placeholder, full }) => (
                  <div key={key} style={full ? { gridColumn: '1 / -1' } : {}}>
                    <label style={{ display: 'block', fontSize: 11, color: 'rgba(255,255,255,0.4)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>{label}</label>
                    <input className="admin-input" style={{ ...inputStyle, width: '100%', boxSizing: 'border-box' }} type={type} placeholder={placeholder} value={editForm[key]} onChange={e => setEditForm(p => ({ ...p, [key]: e.target.value }))} />
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={() => setEditForm({ username: '', newUsername: '', newName: '', newPassword: '' })} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.3)', padding: '12px 20px', borderRadius: 9, fontSize: 13, cursor: 'pointer', fontFamily: "'Source Sans 3', sans-serif" }}>
                  Clear
                </button>
              </div>
            </div>
