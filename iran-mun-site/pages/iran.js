...
function Chatbot() {
  ...
  const [messages, setMessages] = useState([{ role: 'assistant', content: "Hello! I am Sacul, your Iran MUN research assistant. Ask me anything about Iran's position at ECOSOC, MUN procedures, or current events." }])
  ...
  return (
    <>
      <button className="chat-fab" onClick={() => setOpen(o => !o)} title="Ask Iran MUN Assistant">{open ? '✕' : '💬'}{!open && <span className="chat-fab-label">Sacul AI</span>}</button>
      {open && (
        <div className="chat-window">
          <div className="chat-header"><div><div className="chat-header-title">🇮🇷 Lucas: Iran MUN Assistant</div><div className="chat-header-sub">Powered by Groq AI</div></div><button className="chat-close" onClick={() => setOpen(false)}>✕</button></div>
